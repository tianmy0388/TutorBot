...
it("shows loading without simultaneously claiming the profile is empty", () => {
  ...
  expect(screen.getByText("画像加载中…")).toBeInTheDocument();
  expect(screen.queryByText("暂无画像数据")).not.toBeInTheDocument();
});

it("describes the actual profile trigger threshold", () => {
  profileHook.mockReturnValue({
    profile: null,
    loaded: true,
    loading: false,
  ...
  expect(screen.getByText("积累 5 条已评分学习事件，或完成一次效果评估后会建立画像")).toBeInTheDocument();
});
...
it("shows an explicit failure instead of cached profile content after refresh fails", () => {
  ...
  expect(screen.getByText("画像加载失败")).toBeInTheDocument();
