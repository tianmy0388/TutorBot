  ...
  return (
    <div className="mt-3 space-y-3 rounded-lg border border-fg/10 bg-bg-panel/40 p-3">
      ...
      <textarea
        ...
        spellCheck={false}
        className="min-h-52 w-full resize-y rounded-md border border-fg/10 bg-black/50 p-3 font-mono text-xs leading-5 text-fg focus:border-brand-500 focus:outline-none dark:focus:border-fg-muted"
      />
      <div className="flex flex-wrap items-center gap-2">
        <label className="btn-ghost min-h-11 cursor-pointer px-3 py-2 text-xs">
        ...
        <button
          ...
          className={cn(
            "btn-primary min-h-11 px-3 py-2 text-xs",
    ...
    <section aria-label="本次运行结果" className="space-y-2 rounded-md border border-fg/10 bg-bg-card p-3">
      <div className="flex items-center gap-2 text-sm font-medium">
        {attempt.status === "passed" ? (
          <CheckCircle2 className="h-4 w-4 text-green-700 dark:text-fg" />
        ) : (
          <AlertCircle className="h-4 w-4 text-amber-700 dark:text-fg-muted" />
        )}
      ...
      {attempt.stdout && <pre className="max-h-36 overflow-auto whitespace-pre-wrap rounded bg-black/50 p-2 text-xs text-green-700 dark:text-fg">{attempt.stdout}</pre>}
      {attempt.stderr && <pre className="max-h-36 overflow-auto whitespace-pre-wrap rounded bg-black/50 p-2 text-xs text-red-700 dark:text-fg">{attempt.stderr}</pre>}
    </section>
  );
}
...
function Notice({ text, error = false }: { text: string; error?: boolean }) {
  return (
    <div
      className={cn(
        "text-xs text-amber-700 dark:text-fg-muted",
        error && "text-red-700 dark:text-fg-muted",
      )}
    >
      {text}
    </div>
