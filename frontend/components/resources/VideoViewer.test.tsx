...
async function beginRetry() {
  fireEvent.click(
    screen.getByRole("button", { name: "重新生成视频" }),
  );
  await act(async () => {
...
describe("VideoViewer durable render lifecycle", () => {
  ...
  it("uses terminal resource failure when child data is missing or stale", () => {
    ...
    expect(screen.queryByText(/FileNotFoundError/)).not.toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: "查看完整渲染日志" }),
  ...
  it("never renders a legacy raw traceback as the failure summary", () => {
    ...
    expect(
      screen.getByText("视频生成过程中遇到问题，请尝试重新生成。"),
    ).toBeInTheDocument();
  ...
  it("offers a natural regeneration action and preserves the original failure state", async () => {
    ...
    expect(screen.getByText("正在重新生成视频…")).toBeInTheDocument();
    expect(screen.queryByText("视频生成中…")).not.toBeInTheDocument();
    expect(
  ...
  it("keeps video regeneration single-flight while its request is pending", async () => {
    setCanonicalResource(failedResource);
  ...
  it("drops local retry tracking when the matching canonical repair fails", async () => {
    ...
    expect(screen.queryByText("正在重新生成视频…")).not.toBeInTheDocument();
    expect(screen.getByText("重新生成失败")).toBeInTheDocument();
  ...
  it("drops failed local polling when the matching canonical repair becomes ready", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "重新生成视频" }),
    ).not.toBeInTheDocument();
...
describe("VideoViewer initial render sync", () => {
  ...
  it("syncs the stale package once on mount when the initial render child is already terminal", async () => {
    ...
    expect(screen.queryByText("视频已生成")).not.toBeInTheDocument();
    expect(
      screen.queryByText("后台视频任务已成功完成，资源详情正在同步。"),
    ).not.toBeInTheDocument();
