...
TutorBot 是一个面向高等教育学生的 AI 辅助学习系统，通过多智能体协同工作，自动生成针对学生个性化需求的多模态学习资源。核心目标是解决传统教育中"资源繁杂无序、难以精准匹配、缺乏个性化指导"的问题。
...
- **🧩 模块化工作台**：四个一级页面（学习工作台 / 知识库 / 资源中心 / 设置）；视频/PPT 仅在用户显式勾选时生成；普通答疑走快速路径
- **🛡️ 持久化 Job + 强类型结果契约**：`JobResultContract` 终态归一化为 `succeeded / partial / failed / cancelled`；WS 断线/刷新后通过 REST 快照 + 序号游标恢复；引用/置信度/审核/安全/参与 Agent 全部随资源落地
- **🔌 运行时配置**：网页 `/settings` 直接管理 LLM / Embedding / WebSearch 密钥与连接测试；密钥掩码 + 原子 `.env` 写入 + 缓存刷新
- **📤 知识库摄取**：PDF / DOCX / PPTX / Markdown / TXT 五种格式；状态机 `uploaded → extracting → chunking → embedding → ready`；预置 `ai_introduction` 示例库
- **🌐 会话级联网搜索**：聊天框独立开关，新会话默认关闭并按会话持久化；当前项目通过 MiniMax MCP 提供搜索，任务保存来源或明确的降级状态
...
| LLM Provider | 讯飞星火 + DeepSeek + OpenAI-compatible + Anthropic（多 Provider） |
| RAG | LlamaIndex |
...
- **LLM API Key**（讯飞星火 / OpenAI / Anthropic / DeepSeek 任选其一；没有也能跑通,只是部分 Agent 优雅降级）
...
后端的 `TUTOR_PORT` 必须与前端进程的 `BACKEND_PORT` 一致；本仓库两者默认均为 `8000`。
...
在仓库根目录使用当前 Python 环境运行后端测试：

```powershell
python -m pytest backend/tests -q
python -m pytest backend/tests/integration/test_local_validation_follow_up.py -q
```
...
`backend/data/`。请先停止后端，再按照
[本地历史数据迁移与恢复](docs/operations/local-data-migration.md) 执行 dry-run、带时间戳备份、
指定源用户迁移和重启验收。迁移不会删除任一源目录。

### 联网搜索（MiniMax MCP）

联网搜索有两层开关：服务端运行时许可，以及聊天框中按 conversation 保存的“联网搜索”开关。
新建会话的聊天开关默认关闭；刷新和重启会恢复各会话自己的选择。当前项目使用 MiniMax MCP：
...
MiniMax 凭据只应保存在本机环境或 MCP 配置引用的环境变量中，不要提交到 Git 或粘贴到日志。
关闭会话开关时不会发起搜索；开启后，搜索来源会随消息或资源持久化，MCP 不可用则明确降级但不阻塞主任务。
...
- `docs/requirements-analysis.md` — 需求分析与赛题追踪
- `docs/deployment-guide.md` — 部署与运行说明
- `docs/test-report.md` — 测试说明与验收报告
- `docs/open-source-compliance.md` — 开源组件与协议说明
- `docs/ai-coding-disclosure.md` — AI Coding 使用说明
- `docs/privacy-and-safety.md` — 隐私、安全与防幻觉说明
...
检查：
- 后端是否在 `http://localhost:8000` 正常监听（或确认 `TUTOR_PORT` 与前端 `BACKEND_PORT` 完全一致）
