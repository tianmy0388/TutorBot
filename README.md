...
后端的 `TUTOR_PORT` 必须与前端进程的 `BACKEND_PORT` 一致；本仓库两者默认均为 `18000`。
...
# 能力列表
curl http://localhost:18000/api/v1/capabilities
...
[本地历史数据迁移与恢复](docs/operations/local-data-migration.md) 执行 dry-run、带时间戳备份的迁移、
`sess_ebb5a8f5dfdb` 恢复检查和重启验收。迁移只接受规范用户 `local-user`，不会删除任一源目录。
...
检查：
- 后端是否在 `http://localhost:18000` 正常监听（或确认 `TUTOR_PORT` 与前端 `BACKEND_PORT` 完全一致）
